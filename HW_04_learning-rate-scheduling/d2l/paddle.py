...
def plot(X, Y=None, xlabel=None, ylabel=None, legend=None, xlim=None,
         ...
         fmts=('-', 'm--', 'g-.', 'r:'), figsize=(4.9, 3.5), axes=None):
    """绘制数据点
    Defined in :numref:`sec_calculus`"""
    if legend is None:
        legend = []
...
def build_array_nmt(lines, vocab, num_steps):
    ...
    valid_len = d2l.reduce_sum(
       (array != vocab['<pad>']).astype(paddle.int32), 1)
    return array, valid_len
...
def read_voc_images(voc_dir, is_train=True):
    ...
    for i, fname in enumerate(images):
        features.append(paddle.to_tensor(paddle.vision.image.image_load(os.path.join(
            voc_dir, 'JPEGImages', f'{fname}.jpg'), backend='cv2')[..., ::-1], dtype=paddle.float32).transpose([2,0,1]))
        labels.append(paddle.to_tensor(paddle.vision.image.image_load(os.path.join(
            voc_dir, 'SegmentationClass' ,f'{fname}.png'), backend='cv2')[..., ::-1], dtype=paddle.float32).transpose([2,0,1]))
